    ...
    switch(size.len) {
        ...
        case 3:
            for(int i1=0;i1<src->sz[1];i1++){
                for(int i0=0;i0<src->sz[0];i0++){
                    memcpy(array+(size.sz[1]*i0+i1)*size.sz[2],src->p+(src->sz[1]*i0+i1)*src->sz[2],sizeof(TYPE)*src->sz[2]);
                }
            }
            break;
        case 4:
            for(int i2=0;i2<src->sz[2];i2++){
                for(int i1=0;i1<src->sz[1];i1++){
                    for(int i0=0;i0<src->sz[0];i0++){
                        memcpy(array+((size.sz[1]*i0+i1)*size.sz[2]+i2)*size.sz[3],src->p+((src->sz[1]*i0+i1)*src->sz[2]+i2)*src->sz[3],
...
__attribute__((always_inline)) static inline int stringaddvv(ATYPE* dst, ATYPE* op1, ATYPE* op2) {
  if(op1==NULL) {
    reallocMoveArraychar(dst,NULL,maxArraySz(&(__arrayArg){1,{dst->sz[0]}}, &(__arrayArg){1,{op2->sz[0]}}, 0));
